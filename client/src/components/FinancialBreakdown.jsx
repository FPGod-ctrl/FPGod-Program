...
const SECTIONS = [
  ...
  },
  {
    ...
    defaults: { policy_type: 'life', provider: '', product: '', cover_amount: '', premium: '',
      frequency: 'annual', premium_structure: '', owner: '', life_insured: '', inside_super: '',
      waiting_period: '', benefit_period: '', definition: '', features: '', exclusions: '',
      loading: '', start_date: '', action: '', policy_number: '', notes: '' },
    columns: [
      { key: 'policy_type', header: 'Policy', render: (r) => <Badge tone="teal" value={r.policy_type} /> },
      { key: 'product', header: 'Product', render: (r) => r.product || r.provider || '—' },
      { key: 'life_insured', header: 'Life insured', render: (r) => r.life_insured || '—' },
      { key: 'inside_super', header: 'Structure',
        render: (r) => (r.inside_super === true ? 'Inside super' : r.inside_super === false ? 'Outside super' : '—') },
      ...
      { key: 'action', header: 'Action', render: (r) => (r.action ? <Badge tone="amber" value={r.action} /> : '—') },
    ],
    // These map one-to-one onto the house SOA's portfolio table and per-policy
    // write-ups. Anything left blank becomes [ADVISOR TO CONFIRM] in the SOA
    // rather than being guessed, so it is worth filling what you know.
    fields: [
      { key: 'policy_type', label: 'Policy type *', type: 'select',
        options: ['life', 'tpd', 'income_protection', 'trauma', 'health', 'home', 'auto', 'other'] },
      { key: 'provider', label: 'Insurer' },
      { key: 'product', label: 'Product / plan name' },
      { key: 'cover_amount', label: 'Sum insured', type: 'number' },
      { key: 'premium', label: 'Premium', type: 'number' },
      { key: 'frequency', label: 'Premium frequency', type: 'select', options: FREQ },
      { key: 'premium_structure', label: 'Premium structure', type: 'select',
        options: ['stepped', 'level', 'variable age-stepped', 'hybrid'] },
      { key: 'owner', label: 'Policy owner' },
      { key: 'life_insured', label: 'Life insured' },
      { key: 'inside_super', label: 'Held inside super?', type: 'select',
        options: [{ value: 'true', label: 'Inside super' }, { value: 'false', label: 'Outside super' }] },
      { key: 'waiting_period', label: 'Waiting period (IP)' },
      { key: 'benefit_period', label: 'Benefit period (IP)' },
      { key: 'definition', label: 'Definition (e.g. any / own occupation)' },
      { key: 'start_date', label: 'Commenced', type: 'date' },
      { key: 'policy_number', label: 'Policy number' },
      { key: 'action', label: 'Action in this advice', type: 'select',
        options: ['retain', 'replace', 'cancel', 'new', 'reduce', 'increase'] },
      { key: 'features', label: 'Features (buy back, reinstatement, indexation…)', type: 'textarea', full: true },
      { key: 'exclusions', label: 'Exclusions', type: 'textarea', full: true },
      { key: 'loading', label: 'Loadings' },
      { key: 'notes', label: 'Notes', type: 'textarea', full: true },
    ],
  },
  {
